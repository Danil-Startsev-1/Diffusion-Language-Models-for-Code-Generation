...
TOKENROOT = Path("/home/poalivanova/tokenization_miem_hpc")
D4_PATH = TOKENROOT / "B_norm" / "D4" / "train.parquet"

OUTPUT_ROOT = "/home/dstartsev/checkpoint/D4"
RUN_NAME = "stable-diffcoder_D4_1epoch"
...
def main():
    ...
    print("=" * 80, flush=True)
    print("Starting Stable-DiffCoder LoRA fine-tuning on D4", flush=True)
    print(f"Model path: {MODEL_PATH}", flush=True)
    print(f"Dataset path: {D4_PATH}", flush=True)
    print(f"Output dir: {OUTPUT_DIR}", flush=True)
    print("=" * 80, flush=True)

    print("Loading parquet with pandas...", flush=True)
    df = pd.read_parquet(D4_PATH)
